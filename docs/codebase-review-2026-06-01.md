...
| `npm test` | Passed | 67 test files, 416 tests passed. |
...
- Completed: F-006 inventory/employee-info column-management helper slice. `getEmployeeInfoColumnManagementAccess` now centralizes action auth, profile lookup, admin fallback, and column-management RPC handling for the inventory and employee-info create/update/delete/reorder column actions while preserving action-specific denial text and real RPC error returns.
- Completed: F-006 employee-info visibility-rule admin helper slice. `getEmployeeInfoAdminActionAccess` now centralizes visibility-rule action auth, profile lookup, admin fallback, and `is_admin` RPC error handling for the update/clear visibility-rule actions while preserving their route-specific redirect messages.
...
- Completed: F-005 task timeline extraction slice. `app/(app)/tasks/taskTimeline.ts` now owns Gantt date parsing, range calculation, ticks, day diffs, today-marker helpers, and Gantt bar layout math, with `app/(app)/tasks/taskTimeline.test.ts` covering empty timelines, backwards due dates, tick spacing, marker bounds, bar percentages, and calendar-day diffs.
...
- Completed: F-005 task subtask hidden-status helper slice. `TasksView` now routes expanded-subtask visibility through `filterSubtasksByHiddenStatus`, keeping optimistic subtask status overrides and hidden-status behavior covered in the task view-model tests instead of inline component logic.
- Completed: F-005 task view UI helper extraction slice. `app/(app)/tasks/taskViewUi.ts` now owns task filter menu placement, task-notes hover placement, task hover state types/constants, status-color/label lookup helpers, table column definitions, visible-column colspan counting, and pagination summary math with focused unit coverage.
- Completed: F-005 shared task-hover helper slice. `lib/taskHover.ts` now owns task hover API URL construction and notes-preview payload normalization for both `TasksView` and `NoteEditorClient`, with focused unit coverage.
...
- Completed: F-005 note-editor formatting helper extraction slice. `lib/noteEditorFormatting.ts` now owns Word-style font options, visible toolbar font option filtering, font-size stepping, toolbar label normalization, and image-float normalization with focused unit coverage.
...
- Completed: F-005 note-editor overlay resolver extraction slice. `lib/noteEditorOverlays.ts` now owns selected-overlay and context-menu overlay resolution helpers, with focused tests for selection ancestry, DOM lookup, coordinate fallback, and unresolved contexts.
- Completed: F-005 note-editor inline helper extraction slice. `lib/noteEditorInline.ts` now owns timestamp parsing, pasted-link normalization, mention handle cleanup, inline text cleanup, task status labels, task-link ID extraction, and task hover popover placement with focused unit coverage.
...
- Completed: F-005 note-editor state helper extraction slice. `app/(app)/_components/NoteEditorStateHelpers.ts` now owns active table-column detection, text alignment detection, missing-image node lookup, selected/current-line text normalization, suggested task title fallback, meta tooltip/review stats derivation, outline heading derivation, overlay label derivation, and the copied-format snapshot type; the table-column contract now lives in plain TS at `lib/noteEditorTableColumns.ts`.
- Completed: F-005 chat client helper extraction slice. `lib/chatClientUtils.ts` now owns message sorting/merging, sync cursors, conversation-member normalization, display labels, reply parsing, snippets, link URL helpers, conversation lookup maps, pinned/muted priority ordering, conversation search/title derivation, search filtering, direct-chat lookup, message lookup maps, unread anchor resolution, and read-receipt derivation/splitting used by `ChatPageClient`, with focused unit coverage.
...
- Completed: F-005 settings page utility extraction slice. `lib/settingsPageUtils.ts` now owns settings notification preference defaults/types, notification update payloads, avatar/status constants, checkbox/status-color/UUID helpers, template search-param normalization, contextual template/task/project/subtask/link URLs, template relationship maps, template custom-field summaries, notification preference normalization, profile display data, status row/section derivation, status action form parsing, preference fallback handling, initials, default content text, user/group summary derivation, and DB error formatting with focused unit coverage.
...
- `npm test -- --run 'app/(app)/tasks/taskViewModel.test.ts'`: passed, 12 tests.
...
- `npm test -- --run lib/noteEditorOverlays.test.ts`: passed, 12 tests.
...
- `npm test -- --run 'app/(app)/_components/NoteEditorStateHelpers.test.ts'`: passed, 8 tests.
- `npm test -- --run lib/chatClientUtils.test.ts`: passed, 12 tests.
- `npm test -- --run lib/socialDetailUtils.test.ts`: passed, 7 tests.
- `npm test -- --run lib/settingsPageUtils.test.ts`: passed, 11 tests.
...
- `npm test -- --run 'app/(app)/tasks/taskTimeline.test.ts'`: passed, 7 tests.
- `npm test -- --run 'app/(app)/tasks/taskViewUi.test.ts'`: passed, 8 tests.
- `npm test -- --run lib/employeeInfoAccess.test.ts`: passed, 11 tests.
...
- `npm test`: passed, 67 files and 416 tests.
- `npm run lint`: passed.
- `npm run build`: passed on Next.js 15.5.18. `/tasks` built at 4.36 kB route JS and 130 kB first load JS after the task view-model/subtask-hidden-status, timeline/bar-layout, task view UI/colspan, shared task-hover helper, and task page utility extractions; `/tasks/[taskId]` built at 3.18 kB route JS and 133 kB first load JS after the task detail utility extraction; `/clients/[clientId]/notes/[noteId]` built at 2.99 kB route JS and 166 kB first load JS after the note-editor state-helper and overlay resolver extractions; `/personal/[pageId]` built at 3.05 kB route JS and 170 kB first load JS; `/projects` built at 10.5 kB route JS and 120 kB first load JS after the project table view-state extraction; `/clients` built at 6.76 kB route JS and 116 kB first load JS after the client table view-state extraction; `/chat` built at 15 kB route JS and 130 kB first load JS after the chat lookup/priority/search/receipt helper extractions; `/social/[pageId]` built at 3.41 kB route JS and 119 kB first load JS after the expanded social detail helper extraction; `/inventory` built at 11.6 kB route JS and 118 kB first load JS, and `/employee-info` built at 11.1 kB route JS and 118 kB first load JS after the shared table utility, editable-cell helper, preference-state, filter/sort pipeline, access-gate helper, column-management helper, and visibility-rule admin helper extractions; `/forms` built at 5.84 kB route JS and 118 kB first load JS after the list pagination slice; `/settings` built at 4.71 kB route JS and 116 kB first load JS after the latest page-edit guard and settings utility/group-summary/template relationship/custom-field/status/profile/preference/notification-update/status-action/URL extraction slices; route-modal prefetch cleanup, note-editor context-menu, overlay, overlay resolver, inline, image, ribbon primitive, state helper/meta stats/outline/overlay-label helpers, suggestion helper, chat client/derived-state helpers, social detail helper, settings utility, task page/detail utility, and task view-model/timeline/view UI/shared task-hover helper extractions plus the CI workflow, contextual task quick-add, and project access guard slices also passed the production build.
...
- `app/(app)/projects/page.tsx:1265`, `app/(app)/schedules/page.tsx:419`, `app/(app)/schedules/[clientId]/page.tsx:1099`, `app/(app)/social/[pageId]/page.tsx:1670`, and `app/(app)/employee-info/page.tsx:1844` use the same route-modal pattern.
...
- `app/(app)/_components/NoteEditorClient.tsx`: 5735 lines after the content, formatting option, context-menu, overlay, overlay resolver, inline hover, image, ribbon primitive, state helper/meta stats/outline/overlay-label, and suggestion helper extractions. The file remains a major F-005 follow-up target even after the overlay insert-default and overlay node-helper extractions.
- `app/(app)/settings/page.tsx`: 4388 lines after the settings page utility, group-summary, template relationship, custom-field summary, notification preference/update-payload, profile/status section, status-action parser, template URL, subtask redirect, and project-template link redirect extraction slices.
- `app/(app)/chat/ChatPageClient.tsx`: 2388 lines after the chat client helper and derived-state extraction slices, down from 2682 in the latest large-file scan.
- `app/(app)/social/[pageId]/page.tsx`: 2193 lines after the latest social detail helper extraction.
- `app/(app)/tasks/TasksView.tsx`: 2346 lines after the task table view-state, persisted filter restore, URL/query, preference form-data, view-model/subtask hidden-status, timeline/bar-layout, UI helper/colspan, and shared task-hover helper extraction slices, down from 2646 after the quick-add UX slice.
- `app/(app)/inventory/page.tsx`: 1250 lines after the shared access-gate and column-management helper extractions.
- `app/(app)/employee-info/page.tsx`: 1978 lines after the shared access-gate, column-management, and visibility-rule admin helper extractions.
...
- Done for the task timeline slice: `app/(app)/tasks/taskTimeline.ts` extracts Gantt date parsing, range, tick, day-diff, today-marker, and bar-layout helpers, and `app/(app)/tasks/taskTimeline.test.ts` covers the current behavior before larger `TasksView` splits.
- Done for the task view-model slice: `app/(app)/tasks/taskViewModel.ts` extracts hidden-status filtering for tasks and expanded subtasks, quick-created task merging, effective task maps, next-subtask due-date recalculation, and board status grouping; `app/(app)/tasks/taskViewModel.test.ts` covers optimistic status filtering, local quick-task visibility, server-precedence map merging, subtask hidden-status visibility, next-subtask due dates, and unknown-status fallback behavior.
- Done for the task view UI helper slice: `app/(app)/tasks/taskViewUi.ts` extracts filter menu placement, task-notes hover placement, task hover state contracts/constants, visible table-column counting, and pagination summary math; `app/(app)/tasks/taskViewUi.test.ts` covers menu widths, viewport clamping, column counts, and pagination edge cases.
- Done for the shared task-hover helper slice: `lib/taskHover.ts` extracts task hover endpoint URL construction and notes-preview normalization; `lib/taskHover.test.ts` covers encoded task IDs and payload cleanup.
...
- Done for the note-editor formatting-helper slice: `lib/noteEditorFormatting.ts` extracts font option lists, visible toolbar font option filtering, font-size stepping, toolbar label normalization, and image-float normalization; `lib/noteEditorFormatting.test.ts` covers the extracted behavior.
...
- Done for the note-editor overlay resolver slice: `lib/noteEditorOverlays.ts` extracts selected-overlay and context-menu overlay resolution helpers; `lib/noteEditorOverlays.test.ts` covers selection ancestry, DOM lookup, coordinate fallback, and unresolved contexts.
- Done for the note-editor inline-helper slice: `lib/noteEditorInline.ts` extracts timestamp parsing, pasted-link validation, mention handle cleanup, inline text normalization, task status labels, task-link ID extraction, and viewport-clamped task hover placement; `lib/noteEditorInline.test.ts` covers the extracted behavior.
...
- Done for the note-editor state-helper slice: `app/(app)/_components/NoteEditorStateHelpers.ts` extracts editor state/selection helpers, copied-format typing, meta tooltip derivation, review stats derivation, outline heading derivation, and overlay label derivation; `app/(app)/_components/NoteEditorStateHelpers.test.ts` covers table-column fallback, text alignment fallback, missing-image position lookup, selected text normalization, suggested task title fallback, meta tooltip text, word-count reading-time math, mention counts, linked task-reference counts, outline label normalization, outline level clamping, and selected/delete overlay labels.
- Done for the chat client helper slices: `lib/chatClientUtils.ts` extracts deterministic message/conversation helpers from `ChatPageClient`; `lib/chatClientUtils.test.ts` covers sorting, merging, sync cursors, user display labels, URL construction, reply parsing, snippets, invalid date handling, conversation title/search text/filtering, direct-chat lookup, message lookup maps, first unread message resolution, and read-receipt derivation/splitting.
...
- Done for the settings page utility slice: `lib/settingsPageUtils.ts` extracts deterministic settings helpers from `app/(app)/settings/page.tsx`; `lib/settingsPageUtils.test.ts` covers defaults/constants, checkbox parsing, status color normalization, status action form parsing, notification update payloads, template search-param normalization, contextual template/task/project/subtask/link URLs, template relationship maps, template custom-field summaries, notification preference normalization, profile display data, status row/section derivation, preference fallbacks, UUID validation, initials, user/group summary derivation, and DB error formatting.
...
- Inventory/employee-info column-management helper slice extended `lib/employeeInfoAccess.ts`; the eight inventory and employee-info column create/update/delete/reorder action gates now share auth, profile lookup, admin fallback, and RPC error handling.
- Employee-info visibility-rule admin helper slice extended `lib/employeeInfoAccess.ts`; the visibility-rule update and clear actions now share admin auth, profile lookup, `is_admin` fallback, and RPC error handling.
...
- Latest unit-test suite now passes 67 files and 416 tests after the quick task, scoped quick task, inline task mutation, recurrence, status-options, task-sorting, shared task creation, admin API/page access, API auth hardening, settings page-edit, project access guard and error coverage, inventory/employee-info access-gate, column-management, and visibility-rule admin coverage, task/project/client table view-state, inventory/employee table utility/editable-cell/filter-sort helpers and preference-state, task preference payloads, persisted task filter restore/payload cloning, task sort-direction and URL-joining extractions, task view-model/subtask-hidden-status/timeline-bar-layout/UI-colspan/shared-hover helpers, quick-read task RPC, logging, security-definer migration guard, note-editor helper/overlay-default/overlay-node/overlay-resolver/state/meta-stat/outline/overlay-label/inline-hover coverage, chat lookup/search/read-receipt helpers, social detail helpers, and settings utility/template search-param/group-summary/template relationship/custom-field/status/profile/preference/notification-update/status-action/URL slices.
...
- Large list/table surfaces include `app/(app)/inventory/InventoryTable.tsx` at 1898 lines after the shared table utility, editable-cell helper, and preference-state extractions, `app/(app)/employee-info/EmployeeInfoTable.tsx` at 1735 lines after the shared table utility, editable-cell helper, and preference-state extractions, `app/(app)/tasks/TasksView.tsx` at 2346 lines after task helper extractions, `app/(app)/projects/ProjectsView.tsx` at 1817 lines after the project table view-state extraction, and `app/(app)/clients/ClientsTable.tsx` at 1185 lines after the client table view-state extraction.
- The inventory page is 1250 lines and the employee-info page is 1978 lines after the shared access-gate, column-management, and visibility-rule admin helper extractions; their table components remain the larger F-012 UI split targets.
