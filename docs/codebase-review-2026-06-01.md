...
| `npm test` | Passed | 67 test files, 412 tests passed. |
...
- `npm test`: passed, 67 files and 412 tests.
- `npm run lint`: passed.
...
- Latest unit-test suite now passes 67 files and 412 tests after the quick task, scoped quick task, inline task mutation, recurrence, status-options, task-sorting, shared task creation, admin API/page access, API auth hardening, settings page-edit, project access guard and error coverage, inventory/employee-info access-gate, column-management, and visibility-rule admin coverage, task/project/client table view-state, inventory/employee table utility/editable-cell/filter-sort helpers and preference-state, task preference payloads, persisted task filter restore/payload cloning, task sort-direction and URL-joining extractions, task view-model/timeline/UI/shared-hover helpers, quick-read task RPC, logging, security-definer migration guard, note-editor helper/overlay-default/overlay-node/overlay-resolver/state/meta-stat/outline/overlay-label/inline-hover coverage, chat lookup/search/read-receipt helpers, social detail helpers, and settings utility/template search-param/group-summary/template relationship/custom-field/status/profile/preference/notification-update/status-action/URL slices.
