...
| `npm test` | Passed | 67 test files, 417 tests passed. |
...
- `npm test -- --run 'app/(app)/tasks/taskViewUi.test.ts'`: passed, 9 tests.
...
- `app/(app)/tasks/TasksView.tsx`: 2339 lines after the task table view-state, persisted filter restore, URL/query, preference form-data, view-model/subtask hidden-status, timeline/bar-layout, UI helper/colspan/card-metadata, and shared task-hover helper extraction slices, down from 2646 after the quick-add UX slice.
...
- Done for the task view UI helper slice: `app/(app)/tasks/taskViewUi.ts` extracts filter menu placement, task-notes hover placement, task hover state contracts/constants, visible table-column counting, reusable card metadata, and pagination summary math; `app/(app)/tasks/taskViewUi.test.ts` covers menu widths, viewport clamping, card metadata, column counts, and pagination edge cases.
