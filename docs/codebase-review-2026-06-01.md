...
- `app/(app)/settings/page.tsx`: 4601 lines after the settings page utility, group-summary, and custom-field URL extraction slices.
...
- Done for the settings page utility slice: `lib/settingsPageUtils.ts` extracts deterministic settings helpers from `app/(app)/settings/page.tsx`; `lib/settingsPageUtils.test.ts` covers defaults/constants, checkbox parsing, status color normalization, template search-param normalization, contextual template URLs, preference fallbacks, UUID validation, initials, user/group summary derivation, and DB error formatting.
...
- `app/(app)/settings/page.tsx` is 4601 lines after the settings utility, group-summary, and custom-field URL extraction slices and still contains many management forms/actions.
