...
export default async function ScheduleTimeOffPage({
  ...
  async function saveCodeAction(formData: FormData) {
    ...
    if (!defaultPaid.ok) {
      await finish(buildTimeOffPath({ year, status, error: defaultPaid.error }));
      return;
    }
    ...
    if (!carryoverCap.ok) {
      await finish(buildTimeOffPath({ year, status, error: carryoverCap.error }));
      return;
    }
    ...
    if (!sortOrder.ok) {
      await finish(buildTimeOffPath({ year, status, error: sortOrder.error }));
      return;
    }
    ...
    if (!expiryMonth.ok) {
      await finish(buildTimeOffPath({ year, status, error: expiryMonth.error }));
      return;
    }
    ...
    if (!expiryDay.ok) {
      await finish(buildTimeOffPath({ year, status, error: expiryDay.error }));
      return;
    }
  ...
  async function setCodeActiveAction(formData: FormData) {
    ...
    if (!uuidRegex.test(codeId)) {
      await finish(buildTimeOffPath({ year, status, error: "Invalid code id" }));
      return;
    }
  ...
  async function saveOverrideAction(formData: FormData) {
    ...
    if (!uuidRegex.test(userId) || !uuidRegex.test(codeId)) {
      await finish(buildTimeOffPath({ year, status, error: "User and code are required" }));
      return;
    }
    const annualPaid = parseIntInput(formData.get("annual_paid_days"), "Annual paid days", { min: 0 });
    if (!annualPaid.ok) {
      await finish(buildTimeOffPath({ year, status, error: annualPaid.error }));
      return;
    }
  ...
  async function deleteOverrideAction(formData: FormData) {
    ...
    const codeId = String(formData.get("code_id") || "").trim();
    if (!uuidRegex.test(userId) || !uuidRegex.test(codeId)) {
      await finish(buildTimeOffPath({ year, status, error: "Invalid override target" }));
      return;
    }
  ...
  async function createRequestAction(formData: FormData) {
    ...
    if (!isIsoDate(startDate) || !isIsoDate(endDate)) {
      await finish(buildTimeOffPath({ year, status, error: "Start and end date are required" }));
      return;
    }
  ...
  async function decideRequestAction(formData: FormData) {
    ...
    if (!uuidRegex.test(requestId)) {
      await finish(buildTimeOffPath({ year, status, error: "Invalid request id" }));
      return;
    }
    if (decision !== "approved" && decision !== "rejected") {
      await finish(buildTimeOffPath({ year, status, error: "Invalid decision" }));
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Configure leave codes, paid entitlements, approvals, and yearly balances.
            </p>
          </div>
          <Link
            href="/schedules"
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-slate-600">
          Pick the Employee Info date column used as employment start date for proration.
        </p>
        <form action={saveSettingsAction} className="mt-4 flex flex-wrap items-end gap-2">
          ...
          <label className="text-xs text-slate-600">
            ...
            <select
              ...
              disabled={!canManage}
              className="mt-1 block min-w-[18rem] rounded-md border border-slate-300 px-2 py-1.5 text-sm disabled:bg-slate-100"
            >
          ...
          {canManage ? (
            <button type="submit" className="rounded-md border border-slate-900 bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">
              Save policy
            </button>
          ) : null}
        </form>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-slate-600">
          Use the form below to add a new code or update an existing one by reusing its code key.
        </p>
              ...
              {codes.length ? (
                codes.map((code) => (
                  <tr key={code.id} className="border-t border-slate-200">
                    ...
                    <td className="px-3 py-2">{code.is_active ? "Active" : "Inactive"}</td>
                    <td className="px-3 py-2">
                      {canManage ? (
                        <form action={setCodeActiveAction}>
                          ...
                          <button type="submit" className="text-xs font-semibold text-slate-700 hover:underline">
        ...
        {canManage ? (
          <form action={saveCodeAction} className="mt-4 grid gap-2 rounded-md border border-dashed border-slate-300 p-3 md:grid-cols-4">
            <input type="hidden" name="year" value={selectedYear} />
            <input type="hidden" name="status" value={selectedStatus} />
            <label className="text-xs text-slate-600">
              Code
            ...
            <label className="text-xs text-slate-600">
              Paid days/year
              <input type="number" name="default_paid_days_per_year" min={0} defaultValue={0} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
            </label>
            <label className="text-xs text-slate-600">
              Carryover cap
              <input type="number" name="carryover_cap_days" min={0} defaultValue={0} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
            </label>
            <label className="text-xs text-slate-600">
              Expiry month
              <input type="number" name="carryover_expiry_month" min={1} max={12} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="1-12" />
            </label>
            <label className="text-xs text-slate-600">
              Expiry day
              <input type="number" name="carryover_expiry_day" min={1} max={31} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="1-31" />
            </label>
            <div className="flex items-end">
              <div className="w-full space-y-2">
                <label className="inline-flex items-center gap-2 text-xs text-slate-700">
                  <input type="checkbox" name="carryover_enabled" />
                  Carryover enabled
                </label>
                <label className="inline-flex items-center gap-2 text-xs text-slate-700">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-slate-900">Entitlement Overrides</h2>
        {canManage ? (
          <form action={saveOverrideAction} className="mt-4 grid gap-2 md:grid-cols-4">
            <input type="hidden" name="year" value={selectedYear} />
            <input type="hidden" name="status" value={selectedStatus} />
            <label className="text-xs text-slate-600">
              User
              <select name="user_id" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
            ...
            <label className="text-xs text-slate-600">
              Code
              <select name="code_id" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
            ...
            <label className="text-xs text-slate-600">
              Annual paid days
              <input type="number" min={0} name="annual_paid_days" defaultValue={0} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
            </label>
            <div className="flex items-end">
              <button type="submit" className="w-full rounded-md border border-slate-900 bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                Save override
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {overrides.length ? (
                    ...
                    <td className="px-3 py-2">
                      {canManage ? (
                        <form action={deleteOverrideAction}>
                          ...
                          <button type="submit" className="text-xs font-semibold text-red-700 hover:underline">Remove</button>
                        </form>
                      ) : null}
                    </td>
                  </tr>
                ...
                <tr>
                  <td className="px-3 py-3 text-slate-500" colSpan={4}>No overrides yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        {canManage ? (
          <div className="mt-4 grid gap-3 rounded-md border border-slate-200 bg-slate-50/60 p-3 md:grid-cols-2">
            <form method="get" className="space-y-2">
              ...
              <label className="block text-xs text-slate-600">
                User
                <select name="preview_user_id" defaultValue={previewUserId} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
                  <option value="">Select user</option>
                  {users.map((user) => (
              ...
              <label className="block text-xs text-slate-600">
                Code
                <select name="preview_code_id" defaultValue={previewCodeId} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
                  <option value="">Select code</option>
                  {codes.map((code) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="text-xs text-slate-600">Start date
                  <input type="date" name="preview_start" defaultValue={previewStart} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
                </label>
                <label className="text-xs text-slate-600">End date
                  <input type="date" name="preview_end" defaultValue={previewEnd} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
                </label>
              </div>
              <button type="submit" className="rounded-md border border-slate-900 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-800">Preview</button>
            </form>
            ...
            <form action={createRequestAction} className="space-y-2">
              ...
              <label className="block text-xs text-slate-600">User
                <select name="target_user_id" defaultValue={previewUserId} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
                  <option value="">Select user</option>
                  {users.map((user) => (
                    <option key={user.id} value={user.id}>{displayName(user)}</option>
                  ))}
              ...
              <label className="block text-xs text-slate-600">Code
                <select name="code_id" defaultValue={previewCodeId} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required>
                  <option value="">Select code</option>
                  {codes.map((code) => (
                    <option key={code.id} value={code.id}>{code.label}</option>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="text-xs text-slate-600">Start date
                  <input type="date" name="start_date" defaultValue={previewStart} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
                </label>
                <label className="text-xs text-slate-600">End date
                  <input type="date" name="end_date" defaultValue={previewEnd} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" required />
                </label>
              </div>
              <label className="block text-xs text-slate-600">Note
                <textarea name="request_note" rows={2} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="Optional context" />
              </label>
              <button type="submit" className="rounded-md border border-slate-900 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-800">Submit request</button>
            </form>
          </div>
        ) : null}

        {previewError ? <p className="mt-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{previewError}</p> : null}
        {previewResult ? (
          <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
            <p>Preview totals: {String(previewResult.total_days || 0)} day(s), {String(previewResult.paid_days || 0)} paid, {String(previewResult.unpaid_days || 0)} unpaid.</p>
          </div>
        ) : null}
              ...
              {filteredRequests.length ? (
                filteredRequests.map((request) => (
                  <tr key={request.id} className="border-t border-slate-200">
                    ...
                    <td className="px-3 py-2">{request.start_date} to {request.end_date}</td>
                    <td className="px-3 py-2 capitalize">{request.status}</td>
                    <td className="px-3 py-2">{toNonNegative(request.approved_paid_days)} / {toNonNegative(request.approved_unpaid_days)}</td>
                    <td className="px-3 py-2">
                      {canManage && request.status === "pending" ? (
                        <div className="space-y-1">
                          <form action={decideRequestAction}>
                            ...
                            <button type="submit" className="text-xs font-semibold text-emerald-700 hover:underline">Approve</button>
                          </form>
                          <form action={decideRequestAction}>
                            <input type="hidden" name="year" value={selectedYear} />
                            <input type="hidden" name="status" value={selectedStatus} />
                            ...
                            <button type="submit" className="text-xs font-semibold text-red-700 hover:underline">Reject</button>
                          </form>
                        </div>
                      ) : (
                ...
                <tr><td className="px-3 py-3 text-slate-500" colSpan={6}>No requests for this filter.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <form method="get" className="inline-flex items-center gap-2">
            ...
            <label className="text-xs text-slate-600">Year
              <input type="number" name="year" min={2000} max={2200} defaultValue={selectedYear} className="ml-1 w-24 rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
            </label>
            <button type="submit" className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">Apply</button>
          </form>
        </div>
        <div className="mt-4 overflow-x-auto rounded-md border border-slate-200">
          <table className="min-w-full text-left text-sm">
