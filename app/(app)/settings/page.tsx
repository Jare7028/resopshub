...
export default async function SettingsPage(props: {
  ...
  const { data: customFieldOptionsRaw, error: customFieldOptionsError } = await supabase
  ...
  const customFields = (
    customFieldsError ? [] : customFieldsRaw || []
  ) as CustomFieldRow[];
  const customFieldOptions = (
    customFieldOptionsError ? [] : customFieldOptionsRaw || []
  ) as CustomFieldOptionRow[];

  const customFieldsByEntity = customFields.reduce<
    Record<CustomFieldEntityType, CustomFieldRow[]>
  >(
    (acc, field) => {
  ...
  const customFieldOptionsByFieldId = customFieldOptions.reduce<
  ...
  async function createCustomField(formData: FormData) {
    "use server";
    const supabase = createSupabaseServerClient();
    ...
    if (!isCustomFieldEntityType(entityTypeRaw)) {
      redirect("/settings?tab=custom-fields&error=Invalid%20entity%20type");
    }
    if (!label) {
      redirect("/settings?tab=custom-fields&error=Field%20label%20is%20required");
    }

    const entityType = entityTypeRaw as CustomFieldEntityType;
    ...
    if (error) {
      const hint = isSupabaseMissingTableError(error)
        ...
        : "";
      redirect(
        `/settings?tab=custom-fields&error=${encodeURIComponent(`${error.message}${hint}`)}`
      );
    }
    ...
    revalidatePath("/projects");
    revalidatePath("/tasks");
    redirect("/settings?tab=custom-fields&success=Custom%20field%20added");
  }

  async function deleteCustomField(formData: FormData) {
    "use server";
    const supabase = createSupabaseServerClient();
    const id = String(formData.get("id") || "").trim();
    if (!id || !isUuid(id)) {
      redirect("/settings?tab=custom-fields&error=Missing%20field%20id");
    }

    const { error } = await supabase.from("custom_fields").delete().eq("id", id);
    if (error) {
      redirect(`/settings?tab=custom-fields&error=${encodeURIComponent(error.message)}`);
    }
    ...
    revalidatePath("/projects");
    revalidatePath("/tasks");
    redirect("/settings?tab=custom-fields&success=Custom%20field%20deleted");
  }

  async function createCustomFieldOption(formData: FormData) {
    "use server";
    const supabase = createSupabaseServerClient();
    ...
    if (!value) {
      redirect("/settings?tab=custom-fields&error=Option%20value%20is%20required");
    }

    const field = customFields.find((row) => row.id === fieldId);
    if (!field) {
      redirect("/settings?tab=custom-fields&error=Unknown%20field");
    }
    if (field.field_kind !== "dropdown") {
      redirect("/settings?tab=custom-fields&error=Field%20is%20not%20a%20dropdown");
    }
    ...
    const { data: lastOption } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    const nextPosition = (lastOption?.position || 0) + 1;
    ...
    revalidatePath("/projects");
    revalidatePath("/tasks");
    redirect("/settings?tab=custom-fields&success=Dropdown%20option%20added");
  }

  async function deleteCustomFieldOption(formData: FormData) {
    "use server";
    const supabase = createSupabaseServerClient();
    const id = String(formData.get("id") || "").trim();
    if (!id || !isUuid(id)) {
      redirect("/settings?tab=custom-fields&error=Missing%20option%20id");
    }
    ...
    revalidatePath("/projects");
    revalidatePath("/tasks");
    redirect("/settings?tab=custom-fields&success=Dropdown%20option%20deleted");
  }
      ...
      {activeTab === "custom-fields" ? (
        <section className="rounded-lg border border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-6 py-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Add reusable fields for client, project, and task details.
            </p>
          </div>
          <div className="p-6 space-y-6">
            {(customFieldsError && isSupabaseMissingTableError(customFieldsError)) ||
            (customFieldOptionsError && isSupabaseMissingTableError(customFieldOptionsError)) ? (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                Custom fields are not set up yet. Run `sql/custom_fields.sql` in Supabase
            ...
            {(
              [
                ...
                ["task", "Task fields"],
              ] as Array<[CustomFieldEntityType, string]>
            ).map(([entityType, heading]) => (
              <section key={entityType} className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <h3 className="text-sm font-semibold text-slate-900">{heading}</h3>
                <form action={createCustomField} className="mt-3 grid gap-2 md:grid-cols-12">
                  ...
                  <button
                    type="submit"
                    className="md:col-span-3 rounded-md btn-primary px-3 py-2 text-sm font-semibold text-white"
                  >
                    Add field
                  </button>
                </form>
                ...
                <div className="mt-4 space-y-3">
                  {customFieldsByEntity[entityType].length ? (
                    customFieldsByEntity[entityType].map((field) => (
                      <div key={field.id} className="rounded-md border border-slate-200 bg-white p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">{field.label}</p>
                            <p className="text-xs text-slate-500">
                              {field.field_kind === "dropdown" ? "Dropdown" : "Text"} · key:{" "}
                              {field.key}
                            </p>
                          </div>
                        ...
                        {field.field_kind === "dropdown" ? (
                          <div className="mt-3 space-y-2">
                            <div className="flex flex-wrap gap-2">
                              {(customFieldOptionsByFieldId[field.id] || []).length ? (
                                (customFieldOptionsByFieldId[field.id] || []).map((option) => (
                                  <form
                                    key={option.id}
                                    action={deleteCustomFieldOption}
                                  ...
                                  >
                                    ...
                                    <button
                                      type="submit"
                                      className="rounded-sm border border-red-200 bg-red-50 px-1.5 py-0.5 text-[10px] font-semibold text-red-700 hover:bg-red-100"
                                    >
                                      x
                                    </button>
                                  </form>
                            ...
                            <form action={createCustomFieldOption} className="flex gap-2">
