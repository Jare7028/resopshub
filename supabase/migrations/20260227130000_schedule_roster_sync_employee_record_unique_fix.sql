-- Fix remaining roster sync collisions on (client_id, employee_info_record_id)
-- by running cleanup and upserts in ordered phases and serializing per-client sync.
...
declare
  ...
  role_column_id uuid;
  employee_upsert_with_user_count integer := 0;
  employee_upsert_without_user_count integer := 0;
  membership_upsert_count integer := 0;
...
begin
  ...
  upsert_employee_rows_with_user as (
    ...
    select
      ...
      chosen.record_id,
      coalesce(nullif(trim(chosen.full_name), ''), coalesce(nullif(trim(u.full_name), ''), u.email, 'Employee')),
      u.email,
  ...
  into employee_upsert_with_user_count
  from upsert_employee_rows_with_user;
  ...
  upsert_employee_rows_without_user as (
    ...
    select
      ...
      null,
      es.record_id,
      coalesce(nullif(trim(es.full_name), ''), 'Employee'),
      null,
  ...
  select coalesce(count(*), 0)
  into employee_upsert_without_user_count
  from upsert_employee_rows_without_user;

  with upsert_client_memberships as (
    insert into public.schedule_roster_entries (
      client_id,
      user_id,
      employee_info_record_id,
  ...
  select coalesce(count(*), 0)
  into membership_upsert_count
  from upsert_client_memberships;

  upserted_count :=
    employee_upsert_with_user_count
    + employee_upsert_without_user_count
