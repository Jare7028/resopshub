do $$
begin
  if to_regclass('public.forms') is null
    or to_regclass('public.form_submissions') is null
  then
    raise notice 'Skipping forms_list_page because Forms tables are not installed.';
    return;
  end if;
